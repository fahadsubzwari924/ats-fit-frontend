...
import { tButtonVariant, tButtonSize, tButtonType, tIconAlign, tIconType } from './types/button.types';
...
export class ButtonComponent {
  ...
  get computedClasses(): string[] {
    ...
    if (this.customClass) {
      classes.push(...this.customClass.split(' ').filter(Boolean));
    } else {
      classes.push(...BaseClasses);
      classes.push(...(SizeClasses[this.size] || SizeClasses['md']));
      classes.push(...(VariantClasses[this.variant] || VariantClasses['primary']));
    }
    if (this.fullWidth) {
      classes.push('w-full');
    }
  ...
  getIconSvg(): string {
    const icons: Record<string, string> = {
      crown: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m2 4 3 12h14l3-12-6 4-4-8-4 8-6-4zm3 16h14"/></svg>`,
      ...
      cross: `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>`,
